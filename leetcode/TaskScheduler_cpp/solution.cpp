...
class Solution {
    public:
        int leastInterval(vector<char>& tasks, int n) {
            ...
            std::unordered_map<char, int> cooldown;
            for (size_t i = 0; i < tasks.size(); ++i) {
                freq[tasks[i]] += 1;
                cooldown[tasks[i]] = 0;
            }

            std::vector<pair<int, char>> freq_sorted;
            for (auto i : freq) {
                freq_sorted.push_back(make_pair(i.second, i.first));
            }
            sort(freq_sorted.rbegin(), freq_sorted.rend());

            int time = 0;
            while (freq_sorted.front().first) {
                for (size_t i = 0; i < freq_sorted.size(); ++i) {
                    char task = freq_sorted[i].second;
                    if (cooldown[task] == 0) {
                        cooldown[task] = n+1;
                        freq_sorted[i].first --;
                        break;
                ...
                for (size_t i = 0; i < freq_sorted.size(); ++i) {
                    if (cooldown[freq_sorted[i].second] > 0) {
                        cooldown[freq_sorted[i].second] --;
                    }
                }
                time ++;
                sort(freq_sorted.rbegin(), freq_sorted.rend());
...
int main() {
    ...
    int ans = s.leastInterval(tasks, 1);
    cout << ans << endl;
